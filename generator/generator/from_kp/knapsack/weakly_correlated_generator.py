...
class WeaklyCorrelatedGenerator(KPGenerator):
    def generate(self, num_items: int, instance_num: int, instance_tot: int) -> KPInstance:
        ...
        profit = [randint(int(w - self.range_ub / 10), int(w + self.range_ub / 10)) for w in weight]
        profit = [p if p >= 1 else 1 for p in profit]
        tot_w = sum(weight)
